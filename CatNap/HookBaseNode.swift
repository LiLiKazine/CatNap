...
class HookBaseNode: SKSpriteNode, EventListenerNode {
    ...
    func didMoveToScene() {
        ...
        let hookPositon = CGPoint(x: hookNode.position.x, y: hookNode.position.y + hookNode.size.height/2)
        let ropeJoint = SKPhysicsJointSpring.joint(withBodyA: physicsBody!, bodyB: hookNode.physicsBody!, anchorA: position, anchorB: hookPositon)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(catTapped), name: Notification.Name(rawValue: CatNode.kCatTappedNotification), object: nil)
    }
